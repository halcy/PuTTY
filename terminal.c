...
static void power_on(Terminal *term, int clear)
{
    ...
    if (!term->utf && term->ucsdata->utf8_locale)
	term->utf = 1;
...
static void term_out(Terminal *term)
{
	...
	if ((c & ~0x1F) == 0 && term->termstate < DO_CTRLS) {
		...
		if (c>='0' && c<='9') { /* Don't use isdigit, not a char */
			...
			if (term->esc_args[0] < 256)
			    c = CSET_SCOACS|term->esc_args[0];
			else
			    c = term->esc_args[0];
			term_out_litchar(term, c);
			break;
			...
			if(term->esc_nargs == 2 && term->esc_args[0] == 0) {
			    /* UTF-8 has many codepage values.
			     * 65001 at Microsoft
...
static int lastrun = 0;
	...
	for (j = 0; j < term->cols; j++) {
	    ...
	    if ((tattr & ATTR_DIM) && (tattr & ATTR_BOLD)) {
		tattr &= ~(ATTR_DIM|ATTR_BOLD);
	    }
